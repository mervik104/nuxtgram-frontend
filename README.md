...
- **Nuxt SPA** — интерфейс. `ssr: false`, hash-роутер (`hashMode: true`), поэтому годится для статического хостинга (GitHub Pages: статический SPA-билд `_nuxt/*` + компактный `index.html`).
...
- **Cloudflare R2** — объектное хранилище картинок; обращения только через Worker (пресайн-URl, привязка объекта к владельцу), прямых публичных загрузок в bucket нет.
...
| Build | Nuxt (статический SPA-выход: компактный `index.html` + hashed-чанки `_nuxt/*`), `vue-tsc` |
| Тесты | `bun:test` (`tests/`) |

**Migration/History:** раньше Nuxtgram использовал self-hosted Payload CMS (Next.js + MongoDB) как бэкенд. Он удалён из активного проекта и сохранён отдельно в архиве — см. legacy-репозиторий `nuxtgram-backend`.
...
- **Cloudflare R2** — объектное хранилище (bucket `nuxtgram`, endpoint и публичный base-URL в `worker/wrangler.jsonc`).
- **Cloudflare Worker** — вся файловая логика:
  - валидация JWT и ownership (владелец = `sub`), размеры через `MAX_UPLOAD_BYTES`;
  - presigned-URLs на загрузку/скачивание (TTL `S3_SIGNED_URL_TTL_SECONDS`);
  - коррекция метаданных после загрузки, удаление файла/аватара, media/avatar-строки в БД.
- Картинки хранятся **только server-side** по объектным ключам; публичные URL отдаются через R2 base + ключ, пресайн-URLы — только авторизованным.
...
Public vars: `CLERK_ISSUER`, `CLERK_JWKS_URL`, `CLERK_AUDIENCE`, `SURREALDB_URL/NAMESPACE/DATABASE`, `S3_ENDPOINT`, `S3_BUCKET_NAME`, `S3_PUBLIC_BASE_URL`, `S3_SIGNED_URL_TTL_SECONDS`, `S3_REGION` (для R2 — `auto`), `MAX_UPLOAD_BYTES`.

**Secrets (хранятся в `wrangler secret put`, не коммитятся):** `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `SURREALDB_SERVICE_USERNAME`, `SURREALDB_SERVICE_PASSWORD`.
...
bun run generate       # nuxt generate — статический SPA-выход (.output/public) для GH Pages
...
- **R2 security** — доступ к объектам только через Worker; credentials не публикуются и не попадают в frontend.
- **Secrets** — ни один секрет не хранится в коде/репо фронтенда (см. `.env.example`, `.gitignore`).

## Migration Status

Nuxtgram мигрировал с legacy Payload CMS (self-hosted backend: Next.js + MongoDB) на serverless-архитектуру: Nuxt SPA + Clerk + Cloudflare Worker + SurrealDB + Cloudflare R2.
